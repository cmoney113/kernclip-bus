...
func connect() (net.Conn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("kernclip-busd not running — start it with: %s", g("kc-bus start"))
	}
	return c, nil
}
...
func cmdPub(args []string) {
	if len(args) < 1 {
		fatal("usage: kc-bus pub <topic> [data] [--mime type] [--file path]\n       echo data | kc-bus pub <topic>")
	}
	topic := args[0]
	mimeType := "text/plain"
	var data string
...
func cmdGet(args []string) {
	if len(args) < 1 {
		fatal("usage: kc-bus get <topic> [--after N] [--json] [--out path]")
	}
	topic := args[0]
	jsonOut := false
	afterSeq := -1
...
func cmdSub(args []string) {
	if len(args) < 1 {
		fatal("usage: kc-bus sub <topic> [--pattern <glob>] [--topics a,b,c] [--json] [--data-only] [--out dir]")
	}
	topic := ""
	pattern := ""
	var topics []string
...
func cmdCreateTopic(args []string) {
	if len(args) < 1 {
		fatal("usage: kc-bus create-topic <topic> [--ring N]")
	}
	topic := args[0]
	ringSize := 0
	for i := 1; i < len(args); i++ {
...
func cmdTail(args []string) {
	...
	if topic == "" {
		fatal("usage: kc-bus tail <topic> [-n N] [--json]")
	}
...
func cmdWait(args []string) {
	...
	if topic == "" {
		fatal("usage: kc-bus wait <topic> [--timeout ms] [--data-only] [--json]")
	}
...
// cmdPasteOut implements: kc-bus paste > /tmp/gtt-universal.pipe | jq -R '{operation: "typeText", text: .}'
...
// cmdPasteIt runs: gttuni --type "$(kc-bus paste)" with ANSI stripped
...
func showHelp() {
	...
	l("")
	lf("%s — Local IPC message bus for any process, any language, any AI model", b("⬡ kc-bus"))
	l(d("KernClip Message Bus | zero deps | any language | any AI"))
	l("")
	l(b("COMMANDS"))
	lf("  %-40s %s", g("kc-bus pub <topic> <data>"),          d("publish a message to a topic"))
	lf("  %-40s %s", g("echo data | kc-bus pub <topic>"),     d("publish from stdin"))
	lf("  %-40s %s", g("kc-bus pub <topic> --file <path>"),   d("publish a file (image, pdf, binary…)"))
	lf("  %-40s %s", g("kc-bus get <topic>"),                 d("read the latest message"))
	lf("  %-40s %s", g("kc-bus get <topic> --out <path>"),    d("save a received file to disk"))
	lf("  %-40s %s", g("kc-bus sub <topic>"),                 d("stream all new messages (blocking)"))
	lf("  %-40s %s", g("kc-bus sub <topic> --out <dir>"),     d("stream + auto-save incoming files"))
	lf("  %-40s %s", g("kc-bus tail <topic>"),                d("print last 10 messages and exit"))
	lf("  %-40s %s", g("kc-bus wait <topic>"),                d("block until next message arrives"))
	lf("  %-40s %s", g("kc-bus copy <text>"),                 d("copy text to the bus clipboard"))
	lf("  %-40s %s", g("kc-bus paste"),                       d("print the current clipboard value"))
	lf("  %-40s %s", g("kc-bus paste-out"),                   d("type clipboard into active window via gtt pipe"))
	lf("  %-40s %s", g("kc-bus paste-it"),                    d("type clipboard into active window via gttuni (true paste)"))
	lf("  %-40s %s", g("kc-bus pfile"),                       d("save clipboard to ~/kcpaste/pasteNNNN.txt (auto-increment, ANSI-stripped)"))
	lf("  %-40s %s", g("kc-bus topics"),                      d("list all active topics"))
	lf("  %-40s %s", g("kc-bus status"),                      d("check if daemon is running"))
	lf("  %-40s %s", g("kc-bus start"),                       d("start the bus daemon"))
	lf("  %-40s %s", g("kc-bus stop"),                        d("stop the bus daemon"))
	l("")
	l(b("FLAGS"))
	...
	l(b("QUICK START"))
	l("  " + p("kc-bus start"))
	l(`  ` + p(`kc-bus pub greetings "hello world"`))
	l("  " + p("kc-bus get greetings"))
	l("  " + p("kc-bus sub greetings"))
	l("")
	l(b("FILES & IMAGES"))
	l("  " + d("# Send any file — MIME type is detected automatically from the extension:"))
	l("  " + p(`kc-bus pub screenshots --file /tmp/screen.png`))
	l("  " + p(`kc-bus pub documents  --file ~/report.pdf`))
	l("  " + p(`kc-bus pub audio      --file recording.wav`))
	l("")
	l("  " + d("# Receive — saves to the original filename in the current directory:"))
	l("  " + p(`kc-bus get screenshots`))
	l("")
	l("  " + d("# Receive — save to a specific folder:"))
	l("  " + p(`kc-bus get screenshots --out ~/Downloads/`))
	l("")
	l("  " + d("# Watch a topic and auto-save every incoming file as it arrives:"))
	l("  " + p(`kc-bus sub screenshots --out ~/incoming/`))
	l("")
	l(b("CLIPBOARD"))
	l("  " + d("# Copy text to the bus clipboard (replaces any previous value):"))
	l(`  ` + p(`kc-bus copy "meeting at 3pm"`))
	l("  " + p(`echo "some text" | kc-bus copy`))
	l(`  ` + p(`kc-bus copy --file notes.txt`))
	l("")
	l("  " + d("# Paste — prints whatever is currently on the clipboard:"))
	l("  " + p(`kc-bus paste`))
	l("  " + p(`kc-bus paste > output.txt          # redirect to file`))
	l("")
	l("  " + d("# Type — injects clipboard into the active window via gttd (no system clipboard):"))
	l("  " + p(`kc-bus type`))
	l("  " + p(`kc-bus type --pipe /tmp/gtt-wlroots.pipe  # alternate pipe`))
	l("  " + d("# Typical workflow — copy something then type it anywhere:"))
	l(`  ` + p(`kc-bus copy "some text" && kc-bus type`))
	l("")
	l(b("HISTORY & WAITING"))
	l("  " + d("# See the last 10 messages on a topic (like unix tail):"))
	l("  " + p(`kc-bus tail events`))
	l("  " + p(`kc-bus tail events -n 25           # last 25`))
	l("")
	l("  " + d("# Block a script until something publishes to a topic:"))
	l("  " + p(`kc-bus wait job.done               # waits forever`))
	l("  " + p(`kc-bus wait job.done --timeout 5000  # give up after 5 seconds`))
	l("  " + p(`result=$(kc-bus wait job.done --data-only)  # capture output in a script`))
	l("")
	...
	l("")
	l("  " + d("Examples:"))
	l("  " + p(`kc-bus pub gtt.settings.brightness 50`))
	l("  " + p(`kc-bus pub gtt.settings.volume 30`))
	l("  " + p(`kc-bus pub gtt.settings.nightlight true`))
	l("  " + p(`kc-bus pub gtt.system.service "restart:nginx"`))
	l("  " + p(`kc-bus pub gtt.system.notify "Alert|CPU is above 90%"`))
	l("  " + p(`kc-bus sub gtt.system.metrics --data-only | jq .cpu_percent`))
	l("  " + p(`kc-bus wait gtt.system.ack.settings_brightness --data-only`))
	l("")
	l(b("SUBSCRIPTIONS"))
	l("  " + d("Three subscription modes:"))
	l("  " + p(`kc-bus sub <topic>                    `) + d("exact topic (original)"))
	l("  " + p(`kc-bus sub --pattern "agent.*"       `) + d("wildcard — * matches one segment"))
	l("  " + p(`kc-bus sub --pattern "agent.#"       `) + d("wildcard — # matches one+ segments (terminal)"))
	l("  " + p(`kc-bus sub --topics a,b,c            `) + d("multi-topic fan-in stream"))
	l("")
	l(b("TOPIC MANAGEMENT"))
	l("  " + p(`kc-bus create-topic <topic>          `) + d("create with default ring size"))
	l("  " + p(`kc-bus create-topic <topic> --ring 4096`) + d("create with explicit ring size"))
	l("  " + p(`kc-bus manifest                      `) + d("full daemon schema + topic inventory"))
	l("")
	l(b("WIRE PROTOCOL"))
